...
class TestDecisionEngine:
    ...
    @pytest.mark.parametrize(
        "stimulus,drive_tension,expected_tool",
        [
            # Абстрактные запросы
...
class TestEmotionalSupport:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "text,expected_mood,expected_valence_sign",
        [
            # Sad
            ("мне плохо", "sad", -1),
    ...
    async def test_emotion_analysis(self, support, text, expected_mood, expected_valence_sign):
        ...
        assert (state.valence > 0) == (expected_valence_sign > 0) or (
            abs(state.valence) < 0.1 and expected_valence_sign == 0
        )
    ...
    async def test_support_response_generation(self, support):
        ...
        assert (
            "слушаю" in response.lower()
            or "рядом" in response.lower()
            or "выслушать" in response.lower()
        )
