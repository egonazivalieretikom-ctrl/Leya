...
class SoulCryptoManager:
    ...
    def update_file(self, filename: str, new_content: str, metadata: Optional[dict] = None) -> None:
    ...
    def update_all(self, new_soul: dict[str, str], metadata: Optional[dict] = None) -> None:
    ...
    def rollback(self, version_index: int) -> None:
        ...
        if version_index < 0 or version_index >= len(self._history):
            raise SoulVersionError(
                f"Невалидный индекс версии: {version_index}. "
                f"Доступно версий: {len(self._history)}"
            )
    ...
    def _verify_hmac(self, path: Path, content: str) -> None:
        ...
        if not hmac_path.exists():
            ...
            logger.warning(
                f"HMAC-файл отсутствует для {path.name}. "
                f"Создаю подпись при первом чтении."
            )
            self._sign_file(path, content)
            return
        ...
        if not hmac.compare_digest(stored_hmac, computed_hmac):
            ...
            logger.error(
                f"🚨 ОБНАРУЖЕНА ПОДМЕНА soul-файла: {path.name}! "
                f"HMAC не совпадает."
            )
            raise SoulTamperError(
                f"HMAC-подпись не совпадает для {path.name}. "
                f"Файл мог быть подменён.",
