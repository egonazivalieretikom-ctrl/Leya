...
class MemoryConfig:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise LeyaConfigError(f"Не удалось создать brain_dir: {path}", context={"path": str(path), "error": str(exc)}) from exc
        if not os.access(path, os.W_OK):
            raise LeyaConfigError(f"brain_dir недоступен для записи: {path}", context={"path": str(path)})
        if not (0.0 < self.forgetting_threshold < 1.0):
            raise LeyaConfigError("forgetting_threshold должен быть в (0.0, 1.0)", context={"value": self.forgetting_threshold})
        if self.metabolism_interval_seconds <= 0:
            raise LeyaConfigError("metabolism_interval_seconds должен быть > 0", context={"value": self.metabolism_interval_seconds})
...
class WorkspaceConfig:
    ...
    priority_weights: dict = field(default_factory=lambda: {"critical": 0.4, "urgency": 0.3, "drive_relevance": 0.3})
...
class ConstitutionalConfig:
    ...
    allowed_python_modules: list = field(default_factory=lambda: ["math", "json", "re", "datetime", "collections"])
...
class LeyaConfig:
    ...
    def print_summary(self):
        ...
        logger.info(f"Memory: {self.memory.brain_dir} | HMAC: {'SET' if self.memory.hmac_key else 'EMPTY'}")
