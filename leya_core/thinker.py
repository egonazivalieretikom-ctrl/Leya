...
class CognitiveOutput(BaseModel):
    ...
    response: str = Field(
        default="Я обрабатываю информацию...", description="Внешний ответ пользователю"
    )
    internal_monologue: str = Field(default="Анализ стимула...", description="Внутренний монолог")
    action_intent: ActionIntent = Field(
        default=ActionIntent.RESPOND, description="Намерение действия"
    )
    tool_call: ToolCall | None = Field(default=None, description="Вызов инструмента")
